      ...
      <P.Wrapper>
        ...
        <P.ImagesWrapper>
          {props.data?.fetchUseditem?.images
            ?.filter((el: string) => el)
            .map((el: string) => (
              <P.Images key={el} src={`https://storage.googleapis.com/${el}`} />
            ))}
        </P.ImagesWrapper>
        <P.Name>{props.data?.fetchUseditem?.name}</P.Name>
        <P.Remarks>{props.data?.fetchUseditem?.remarks}</P.Remarks>
        <P.Price>{getPrice(props.data?.fetchUseditem?.price)}원</P.Price>
        <P.Contents>{props.data?.fetchUseditem?.contents}</P.Contents>
        <P.Address>
        ...
        <>
          <P.Button onClick={props.onClickModalOpenUsePoint}>구매하기</P.Button>
